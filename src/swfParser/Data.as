...
//			var val:String="";//原
			var char:uint;
			var byte:ByteArray=new ByteArray(); //cwh
			while ((char=readUnsignedByte()) != 0)
			{
				byte.writeByte(char); //cwh
//				val+=String.fromCharCode(char);//原
			}
			byte.position=0; //cwh
			return (byte.readMultiByte(byte.length, "UTF-8")); //cwh
//			return val;//原
		}
